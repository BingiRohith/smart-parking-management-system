...
- **Status:** ✅ **FIXED, with an honest caveat about how it was verified.** Added `SIGTERM`/`SIGINT` handlers that call `server.close(callback)` and, inside that callback (once in-flight requests drain), `mongoose.connection.close()`, then exit cleanly — the standard, universally-used Node.js graceful-shutdown pattern. **Verification limitation:** this development environment is Windows, and Node.js on Windows does not deliver POSIX-style signals to a child process's JS-level `process.on('SIGTERM'/'SIGINT', ...)` handlers when sent programmatically via `child_process.kill()` (confirmed directly: even a trivial standalone script with nothing but a bare `SIGINT` handler never received the signal when killed this way on this machine) — this is a documented Node/Windows platform limitation, not specific to this code. The actual target for this fix (containers under Docker/Kubernetes/PM2, sending real POSIX signals on Linux) is unaffected by this Windows-only testing gap. Confirmed the server still boots and serves requests normally with the new code present (no regression), but the shutdown path itself could not be exercised end-to-end in this sandbox.
